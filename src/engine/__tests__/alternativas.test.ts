...
describe("motor de alternativas", () => {
  ...
  // Busca un partido que SI tenga alternativas (a escala, muchos recintos
  // saturan y no las tienen — ese es el caso "martes", cubierto aparte).
  const conAlt = result.asignaciones.find(
    (a) => !proponerAlternativas(a.partidoId, result.asignaciones, input).sinAlternativas,
  )!;
  const partido = input.partidos.find((p) => p.id === conAlt.partidoId)!;

  it("propone alternativas legales, rankeadas por delta de puntaje", () => {
    const r = proponerAlternativas(conAlt.partidoId, result.asignaciones, input);
  ...
  it("aplicar una alternativa no genera choque de recinto", () => {
    ...
    const ocupadas = new Set(
      result.asignaciones
        .filter((a) => a.partidoId !== conAlt.partidoId)
        .map((a) => `${a.recintoId}|${a.fecha}|${a.hora}`),
    );
    expect(ocupadas.has(`${alt.recintoId}|${alt.fecha}|${alt.hora}`)).toBe(false);
  });

  it("sin alternativas legales -> resultado EXPLICITO con razon (caso 'martes')", () => {
    ...
    for (const fecha of [finde.sabado, finde.domingo])
      for (const hora of input.horas)
        if (!(fecha === conAlt.fecha && hora === conAlt.hora)) bloqueos.push({ fecha, hora });
    const r = proponerAlternativas(conAlt.partidoId, result.asignaciones, { ...input, bloqueos });
