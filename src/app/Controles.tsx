...
function Panel({ res }: { res: Respuesta }) {
  ...
  const peorCarga = res.findes.length
    ? Math.max(...res.findes.flatMap((f) => Object.values(f.cargaPorRecinto)))
    : 0;
  const peorMs = res.findes.length
    ? Math.max(...res.findes.map((f) => f.ms ?? 0))
    : null;
  return (
    <div className="mt-4 rounded-lg border border-gray-200 p-4 text-sm">
      <div className="mb-3 flex items-center gap-2">
        <span className={`rounded px-2 py-0.5 text-xs font-semibold ${res.motor === "solver" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>
      ...
      <details className="mt-3">
        <summary className="cursor-pointer text-xs text-gray-500">Desglose de restricciones duras</summary>
      ...
      {res.sinAsignar.length > 0 && (
        <details className="mt-2" open>
          <summary className="cursor-pointer text-xs font-medium text-amber-700">{res.sinAsignar.length} sin cupo (por que)</summary>
          <ul className="mt-2 space-y-0.5 text-xs text-gray-600">
            {res.sinAsignar.slice(0, 12).map((s, i) => (
              <li key={i}>{s.partido} — <b>{s.razon}</b>{s.detalle ? `: ${s.detalle}` : ""}</li>
            ))}
          </ul>
        </details>
      )}
...
function Metrica({ etiqueta, valor, bueno, malo }: { etiqueta: string; valor: string | number; bueno?: boolean; malo?: boolean }) {
  return (
    <div>
      ...
      <div className={`text-lg font-semibold ${malo ? "text-red-600" : bueno ? "text-emerald-600" : "text-gray-800"}`}>{valor}</div>
    </div>
  );
}
